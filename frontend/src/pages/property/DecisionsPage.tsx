...
import { Plus, ThumbsUp, ThumbsDown, PlayCircle, Vote as VoteIcon, FileSignature } from "lucide-react";
...
export default function DecisionsPage() {
  ...
  const [signingDecision, setSigningDecision] = useState<Decision | null>(null);
  ...
  return (
    <div>
              ...
              <div key={d.id} className="card p-4">
                ...
                {d.status === "APPROVED" && canManage && (
                  <div className="mt-3 border-t border-ink-100 pt-3">
                    <button onClick={() => setSigningDecision(d)} className="btn-brass text-sm">
                ...
                {d.status === "EXECUTED" && d.signedByName && (
                  <div className="mt-3 flex items-center gap-2 border-t border-ink-100 pt-3 text-xs text-ink-400">
                    <FileSignature className="h-3.5 w-3.5" />
                    {language === "ar" ? `وقّعه ${d.signedByName}` : `Signed by ${d.signedByName}`}
                  </div>
                )}
              </div>
            );
      ...
      {propertyId && signingDecision && (
        <ExecuteSignatureModal
          decision={signingDecision}
          propertyId={propertyId}
          onClose={() => setSigningDecision(null)}
          onDone={() => {
...
function ExecuteSignatureModal({
  decision,
  propertyId,
  onClose,
  onDone,
  ...
  async function handleSubmit(e: FormEvent) {
    e.preventDefault();
    if (!signatureDataUrl) {
      setError(language === "ar" ? "التوقيع مطلوب." : "Signature is required.");
      return;
    }
    setError(null);
    setLoading(true);
    try {
      await decisionsApi.execute(propertyId, decision.id, signedByName, signatureDataUrl);
      onDone();
    } catch (err) {
      setError((err as Error).message);
    } finally {
  ...
  return (
    <Modal open onClose={onClose} title={language === "ar" ? "تسجيل تنفيذ القرار" : "Record decision execution"}>
      <form onSubmit={handleSubmit} className="space-y-4">
        <ErrorText message={error} />
        <p className="rounded-md bg-parchment-100 p-3 text-sm text-ink-600">
          {language === "ar"
            ? `أنت على وشك تسجيل تنفيذ القرار "${decision.title}". هذا يُنشئ سجل توقيع داخلي كإثبات على من فوّض التنفيذ — وهو ليس توقيعًا إلكترونيًا معتمدًا قانونيًا.`
            : `You're about to record execution of "${decision.title}". This creates an internal signed record of who authorized it — not a certified legal e-signature.`}
        </p>
        <div>
          <label className="label">{language === "ar" ? "الاسم الكامل" : "Full name"}</label>
          <input className="input" required value={signedByName} onChange={(e) => setSignedByName(e.target.value)} />
        </div>
        <div>
        ...
        <button className="btn-brass w-full" disabled={loading}>
          {language === "ar" ? "تأكيد التنفيذ والتوقيع" : "Confirm execution & sign"}
        </button>
      </form>
    </Modal>
  );
