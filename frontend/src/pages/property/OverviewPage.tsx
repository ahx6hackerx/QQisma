...
import { AlertTriangle, Vote, FileWarning, TrendingUp, TrendingDown, Sparkles, Search, Send } from "lucide-react";
...
import type { ActivityItem, Anomaly } from "../../types";
...
export default function OverviewPage() {
  ...
  useEffect(() => {
    ...
    financeApi.anomalies(propertyId).then(setAnomalies);
  ...
  return (
    <div className="space-y-8">
      ...
      {anomalies.length > 0 && (
        <div className="card border-clay-100 bg-clay-50/40 p-5">
          ...
          <div className="space-y-2">
            {anomalies.map((a) => (
              <p key={a.transactionId} className="text-sm text-clay-600">
                {language === "ar"
                  ? `مصروف "${a.category}" هذا الشهر (${formatMoney(a.amount)}) أعلى من المعتاد بنسبة ${a.percentAbove}% (المتوسط ${formatMoney(a.average)}).`
                  : `This month's "${a.category}" expense (${formatMoney(a.amount)}) is ${a.percentAbove}% above the usual average of ${formatMoney(a.average)}.`}
              </p>
            ))}
          </div>
        </div>
...
function AskQismaWidget() {
  ...
  async function handleAsk(e: FormEvent) {
    e.preventDefault();
    if (!propertyId || !question.trim()) return;
    setLoading(true);
    setAnswer(null);
    const result = await answerQuestion(propertyId, question, language);
    setAnswer(result);
    setLoading(false);
  }

  const suggestions =
    language === "ar" ? ["كم بقي لي؟", "نسبة ملكيتي؟", "اعتراضات مفتوحة؟"] : ["How much is left?", "My ownership share?", "Open disputes?"];

  return (
    <div className="card p-5">
      <div className="mb-3 flex items-center gap-2">
        <Search className="h-4 w-4 text-brass-600" />
        <h2 className="font-display text-base font-semibold text-ink-800">
          {language === "ar" ? "اسأل قِسمة" : "Ask Qisma"}
        </h2>
      </div>
      <form onSubmit={handleAsk} className="flex gap-2">
        <input
          className="input"
          placeholder={language === "ar" ? "اكتب سؤالك عن هذا العقار..." : "Ask a question about this property..."}
          value={question}
          onChange={(e) => setQuestion(e.target.value)}
        />
        <button className="btn-outline shrink-0 text-sm" disabled={loading}>
          <Send className="h-4 w-4" />
        </button>
      </form>
      <div className="mt-2 flex flex-wrap gap-1.5">
        {suggestions.map((s) => (
          <button key={s} onClick={() => setQuestion(s)} className="rounded-full bg-parchment-100 px-2.5 py-1 text-xs text-ink-500 hover:bg-parchment-200">
            {s}
          </button>
        ))}
      </div>
      {loading && <p className="mt-3 text-sm text-ink-400">{language === "ar" ? "جارٍ البحث..." : "Looking it up..."}</p>}
      {answer && <p className="mt-3 rounded-md bg-parchment-100 p-3 text-sm text-ink-700">{answer}</p>}
    </div>
  );
}
