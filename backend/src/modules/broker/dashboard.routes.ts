...
brokerDashboardRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const [totalListings, availableListings, totalLeads, activeLeads, recentLeads, dueFollowUps] = await Promise.all([
      ...
      prisma.lead.findMany({
        where: { brokerId, nextFollowUpAt: { lte: new Date() }, status: { notIn: ["WON", "LOST"] } },
        include: { listing: { select: { id: true, title: true } } },
    ...
    res.json({ totalListings, availableListings, totalLeads, activeLeads, recentLeads, dueFollowUps });
  })
);
