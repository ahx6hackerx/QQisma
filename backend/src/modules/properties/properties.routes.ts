...
import { whatsappShareLinkForUser } from "../../utils/notify";
...
// GET /api/properties/:propertyId/partners/:userId/whatsapp-link?message=...
// Zero-setup fallback: builds a wa.me link pre-filled with the given
// message, for a "Send via WhatsApp" button — works today with no Meta
// signup, regardless of whether the Cloud API auto-send is configured.
propertiesRouter.get(
  "/:propertyId/partners/:userId/whatsapp-link",
  requirePropertyMember,
  asyncHandler(async (req, res) => {
    const link = await whatsappShareLinkForUser(req.params.userId, String(req.query.message ?? ""));
    if (!link) throw AppError.badRequest("لا يوجد رقم هاتف مسجّل لهذا الشريك.");
    res.json({ link });
  })
);
